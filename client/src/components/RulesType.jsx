...
function RulesType({ name, url, id }) {
  ...
  useEffect(() => {
    fetch(`https://www.dnd5eapi.co${url}`)
      ...
      .then((data) => {
        setSubsection(data.subsections);
      })
  ...
  return (
    <>
      <h2 className="title-adventuring-h2" key={name} id={id}>
        {name}
      </h2>
      {subsection.map((element) => (
        <ul key={element.url}>
          ...
          >
            ...
            {isOpen && detailRule && detailRule.name === element.name && (
              <div>
                <p className="detailRule">
                  <Markdown>{detailRule.desc}</Markdown>
                </p>
              </div>
            )}
          </button>
